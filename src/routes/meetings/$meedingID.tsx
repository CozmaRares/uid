...
export const Route = createFileRoute('/meetings/$meedingID')({
  component: RouteComponent,
})

function RouteComponent() {
  const { meedingID } = Route.useParams()
  const meeting = meetings.find((meeting) => meeting.id === meedingID)
  ...
  return (
    <div
      key={meedingID}
      className="mx-auto grid max-w-screen-xl grid-cols-1 gap-4 p-4 lg:grid-cols-[5fr,3fr] lg:grid-rows-[60vh,auto] xl:grid-cols-[2fr,1fr]"
    >
...
function Chat({ status, messages }: ChatProps) {
  return (
    <ScrollArea
      className={cn(
        'relative max-h-[400px] rounded-lg border bg-gray-100 p-2 lg:max-h-full',
        status === 'upcoming' && 'bg-gray-100/70',
      )}
    >
      {status === 'upcoming' ? (
